...
if ticker_consulta:
    ...
    if df_periodo.empty:
        ...
        corpo = base.mark_bar(size=3).encode(y="open:Q", y2="close:Q", color=alt.Color("cor:N", scale=None))
        m50  = base.mark_line(color="#4da6ff", strokeWidth=1.3).encode(y="ma50:Q")
        m100 = base.mark_line(color="#b06fe0", strokeWidth=1.3).encode(y="ma100:Q")
        m200 = base.mark_line(color="#ffffff", strokeWidth=1.1).encode(y="ma200:Q")

        chart_preco = alt.layer(vol, pavio, corpo, m50, m100, m200).resolve_scale(y="independent").properties(
        ...
        BENCHMARKS = {
            "IBOV": "^BVSP", "Small Caps (SMLL)": "SMAL11",
            "Dividendos (IDIV)": "DIVO11", "IVVB11 (S&P500)": "IVVB11",
        }
        series_cmp = []
        with st.spinner("Buscando benchmarks..."):
            ...
            for nome_bm, tk_bm in BENCHMARKS.items():
                df_bm = buscar_historico_periodo(tk_bm, cfg["range"], cfg["interval"], token)
                if not df_bm.empty:
        ...
        st.caption("⚠️ CDI e IPCA são estimativas com juros compostos sobre a taxa anual atual (não são séries "
                   "históricas reais). Small Caps, Dividendos e IVVB11 usam ETFs como proxy (SMAL11, DIVO11, IVVB11).")
