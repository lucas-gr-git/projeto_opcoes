...
PERIODOS_CHART = {
    "5 Dias": {"range": "5d",  "interval": "1d"},
    "1 Mês":  {"range": "1mo", "interval": "1d"},
    "3 Meses":{"range": "3mo", "interval": "1d"},
}
# Ranges maiores (6mo, 1y, 2y, 5y, 10y) exigem plano pago na BRAPI — ver aviso na UI.
        ...
        if r.status_code != 200:
            ...
            try:
                ...
                if isinstance(corpo, dict):
                    corpo_msg = corpo.get("message") or corpo.get("error")
            except Exception:
                pass
            return vazio, f"HTTP {r.status_code}" + (f" — {corpo_msg}" if corpo_msg else "")
        data = r.json()
...
with col_periodo:
    ...
    st.caption("⚠️ Limitado a 3 meses pelo plano gratuito da BRAPI — períodos maiores "
               "(6 Meses, 1 Ano, 2 Anos, 5 Anos) exigem plano pago.")
...
if ticker_consulta:
    ...
    if df_periodo.empty:
        ...
        janelas = [("5 dias", 5), ("1 mês", 30), ("3 meses", 91)]
        linha_ret = {}
        for label, dias in janelas:
            r = retorno_ate(df_periodo, dias)
            linha_ret[label] = f"{r:+.2f}%" if r is not None else "-"
        st.dataframe(pd.DataFrame([linha_ret], index=["Rentabilidade"]), width="stretch")
        st.caption("⚠️ Janelas de 1 ano, 2 anos, 5 anos e 10 anos exigem plano pago na BRAPI "
                   "(seu plano atual só libera histórico de até 3 meses).")
        ...
        with st.spinner("Buscando benchmarks..."):
            ...
            for nome_bm, tk_bm in BENCHMARKS.items():
                ...
                if not df_bm.empty and df_bm["close"].iloc[0]:
                    ...
                    # segunda tentativa: range mínimo, pra separar "ticker não existe/plano não cobre"
                    # de "esse range/intervalo específico não é suportado para este ticker"
                    _, motivo_min = buscar_historico_diag(tk_bm, "5d", "1d", token)
                    if motivo_min is None:
                        falharam.append(f"{nome_bm} — falhou com {cfg['range']}/{cfg['interval']} "
                                        f"({motivo}), mas funciona com range curto (5d/1d)")
                    else:
                        falharam.append(f"{nome_bm} — {motivo or 'sem dados'} "
                                        f"(também falhou no teste mínimo: {motivo_min})")
