...
def buscar_macro():
    ...
    for i in range(0, len(todos), 10):
        ...
        symbols = ",".join(lote)
        for base_url in [
            f"https://query1.finance.yahoo.com/v8/finance/quote?symbols={symbols}",
            f"https://query2.finance.yahoo.com/v8/finance/quote?symbols={symbols}",
        ]:
            try:
...
with st.expander("📋 Ver todos os ativos monitorados"):
    ...
    st.dataframe(
        df.style.map(colorir, subset=["Variação"]),
