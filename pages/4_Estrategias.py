...
from rco_core import (
    Mercado, recomendar, black_scholes, gregas,
    gerar_venda_coberta_variacoes, gerar_wheel_variacoes,
    gerar_trava_alta_variacoes, gerar_trava_baixa_variacoes,
    gerar_straddle_strangle_variacoes,
)
from indicators import (
    buscar_historico, calcular_niveis_atr, obter_atr_e_referencia,
    NIVEIS_ATR_PADRAO, REFERENCIA_MAP,
)
...
def historico_diario_cache(ticker, token):
    return buscar_historico(ticker, "Diário", token)

@st.cache_data(ttl=600)
def atr_referencia_cache(ticker, referencia, periodo, token):
    return obter_atr_e_referencia(ticker, referencia, periodo, token)

def grafico_ativo(df, strikes=None, niveis_atr=None, preco_ref=None, altura=420):
    """Candlestick do ativo, com linha de strike (esquerda) e banda de ATR ao
    estilo terminal profissional: Pivot (amarelo) + níveis + ATR (verde acima /
    vermelho abaixo), com rótulos à direita, como na plataforma de referência."""
    if df is None or df.empty:
        st.info("📉 Sem histórico disponível para este ativo agora (verifique o token BRAPI).")
        return

    df = df.copy()
    df["cor"] = np.where(df["close"] >= df["open"], "#2ca02c", "#e05252")
    x_ini, x_fim = df["date"].iloc[0], df["date"].iloc[-1]

    base = alt.Chart(df).encode(x=alt.X("date:T", title=None, axis=alt.Axis(format="%d/%m")))
    pavio = base.mark_rule().encode(
        y=alt.Y("low:Q", title="Preço (R$)", scale=alt.Scale(zero=False)),
        y2="high:Q", color=alt.Color("cor:N", scale=None),
    )
    corpo = base.mark_bar(size=4).encode(
        y="open:Q", y2="close:Q", color=alt.Color("cor:N", scale=None),
        tooltip=[alt.Tooltip("date:T", title="Data", format="%d/%m/%Y"),
                 alt.Tooltip("open:Q", title="Abertura", format=".2f"),
                 alt.Tooltip("high:Q", title="Máxima", format=".2f"),
                 alt.Tooltip("low:Q", title="Mínima", format=".2f"),
                 alt.Tooltip("close:Q", title="Fechamento", format=".2f")],
    )
    camadas = [pavio, corpo]

    # ── Strike(s) da estratégia — linha tracejada azul, rótulo à ESQUERDA ────
    if strikes:
        df_k = pd.DataFrame([{"y": v, "label": f"{k} "} for k, v in strikes.items() if v])
        if not df_k.empty:
            df_k["x"] = x_ini
            camadas.append(alt.Chart(df_k).mark_rule(strokeDash=[5, 3], strokeWidth=1.6, color="#4da6ff").encode(y="y:Q"))
            camadas.append(alt.Chart(df_k).mark_text(align="right", dx=-4, dy=-7, color="#4da6ff",
                                                       fontSize=11, fontWeight="bold")
                            .encode(x="x:T", y="y:Q", text="label:N"))

    # ── Pivot + bandas de ATR — estilo terminal, rótulos à DIREITA ───────────
    if niveis_atr is not None and not niveis_atr.empty:
        rows = []
        if preco_ref is not None:
            rows.append({"y": preco_ref, "label": " Pivot", "cor": "#eab308", "op": 1.0})
        n_niveis = len(niveis_atr)
        for i, (_, row) in enumerate(niveis_atr.sort_values("nivel").iterrows()):
            op = max(1.0 - i * 0.25, 0.4)
            rows.append({"y": row["preco_superior"], "label": f" +{row['rotulo']}", "cor": "#2ca02c", "op": op})
            rows.append({"y": row["preco_inferior"], "label": f" -{row['rotulo']}", "cor": "#e05252", "op": op})
        df_niv = pd.DataFrame(rows)
        df_niv["x"] = x_fim

        for _, r in df_niv.iterrows():
            df_linha = pd.DataFrame([r])
            camadas.append(alt.Chart(df_linha).mark_rule(strokeWidth=1.6, color=r["cor"], opacity=r["op"])
                            .encode(y="y:Q"))
            camadas.append(alt.Chart(df_linha).mark_text(align="left", dx=4, dy=-6, color=r["cor"],
                                                           fontSize=10, fontWeight="bold", opacity=r["op"])
                            .encode(x="x:T", y="y:Q", text="label:N"))

    chart = alt.layer(*camadas).properties(height=altura, background="#0e1117").configure_axis(
...
with tabs[1]:
    ...
    if ticker_sel != "OUTRO":
        st.markdown("**📊 Gráfico do ativo com o strike selecionado**")
        grafico_ativo(historico_diario_cache(ticker_sel, get_token()),
                      strikes={f"Strike venda R$ {K_sel:.2f}": K_sel})
...
with tabs[2]:
    ...
    st.caption("Strike escolhido pelo nível de ATR (suporte projetado) — sem perfis pré-definidos")

    # ── Configurador de Níveis de ATR ────────────────────────────────────────
    if "atr_periodo" not in st.session_state:
        st.session_state.atr_periodo    = 14
        st.session_state.atr_referencia = "Mensal"
        st.session_state.atr_niveis     = [1.0]
        st.session_state.atr_ativo      = True

    col_cfg, col_toggle = st.columns([5, 1])
    with col_toggle:
        st.session_state.atr_ativo = st.toggle("ATR", value=st.session_state.atr_ativo,
                                                key="atr_toggle_vp")

    with col_cfg.expander("⚙️ Configurar Níveis de ATR"):
        st.caption("Ajuste os parâmetros do indicador selecionado")
        with st.form("form_atr_vp"):
            c1, c2 = st.columns(2)
            with c1:
                periodo_novo = st.number_input("Período", min_value=2, max_value=100,
                                                value=st.session_state.atr_periodo, step=1)
            with c2:
                referencia_novo = st.selectbox("Referência", list(REFERENCIA_MAP.keys()),
                    index=list(REFERENCIA_MAP.keys()).index(st.session_state.atr_referencia))
            niveis_novo = st.multiselect("Níveis (± N×ATR a partir do Pivot)", NIVEIS_ATR_PADRAO,
                default=st.session_state.atr_niveis, format_func=lambda n: f"±{n:g}")

            b1, b2, b3 = st.columns(3)
            remover  = b1.form_submit_button("🗑️ Remover", width="stretch")
            cancelar = b2.form_submit_button("Cancelar", width="stretch")
            salvar   = b3.form_submit_button("💾 Salvar", type="primary", width="stretch")

        if salvar:
            st.session_state.atr_periodo    = periodo_novo
            st.session_state.atr_referencia = referencia_novo
            st.session_state.atr_niveis     = niveis_novo
            st.session_state.atr_ativo      = True
            st.rerun()
        if remover:
            st.session_state.atr_ativo = False
            st.rerun()

    token = get_token()
    niveis_df, preco_ref = None, None
    if ticker_sel != "OUTRO" and st.session_state.atr_ativo:
        with st.spinner("Calculando ATR..."):
            _, atr_valor, preco_ref = atr_referencia_cache(
                ticker_sel, st.session_state.atr_referencia,
                st.session_state.atr_periodo, token)
        if atr_valor:
            niveis_df = calcular_niveis_atr(preco_ref, atr_valor, st.session_state.atr_niveis)
            st.caption(f"📐 ATR({st.session_state.atr_periodo}) {st.session_state.atr_referencia} · "
                       f"valor: R$ {atr_valor:.2f} · Pivot: R$ {preco_ref:.2f}")
        else:
            st.warning("⚠️ Histórico insuficiente para calcular o ATR deste ativo/timeframe.")

    # ── Strike a partir do nível de ATR (suporte abaixo do spot) ─────────────
    st.markdown("**Strike (nível de suporte projetado pelo ATR)**")
    if niveis_df is not None and not niveis_df.empty:
        opcoes = {f"−{row['rotulo']}  →  R$ {row['preco_inferior']:.2f}": row["preco_inferior"]
                  for _, row in niveis_df.iterrows()}
        chaves = list(opcoes.keys())
        rotulo_sel = st.radio("Nível ATR usado como strike", chaves, horizontal=True,
                              index=0, label_visibility="collapsed")
        K_sel = opcoes[rotulo_sel]
    else:
        st.info("Ative o ATR (e selecione um ativo cadastrado) para calcular o strike automaticamente, "
                "ou informe manualmente abaixo.")
        K_sel = st.number_input("Strike manual (R$)", min_value=0.01,
                                value=round(S * 0.95, 2), step=0.10, format="%.2f")

    T_v = dias / 365
    premio = black_scholes(S, K_sel, T_v, taxa / 100, vi_pct / 100, "put")
    g = gregas(S, K_sel, T_v, taxa / 100, vi_pct / 100, "put")
    preco_efetivo = K_sel - premio
    desconto_pct = (1 - preco_efetivo / S) * 100 if S else 0

    m1, m2, m3, m4 = st.columns(4)
    m1.metric("Strike", f"R$ {K_sel:.2f}")
    m2.metric("Prêmio", f"R$ {premio:.4f}")
    m3.metric("Preço efetivo", f"R$ {preco_efetivo:.2f}", delta=f"-{desconto_pct:.2f}%")
    m4.metric("Delta / Theta dia", f"{g['delta']:.3f} / {abs(g['theta']):.4f}")

    st.markdown(f"**Payoff: Strike R$ {K_sel:.2f} · {dias} dias · Prêmio R$ {premio:.4f}**")
    spots = [round(S*m,2) for m in [x/100 for x in range(70,121,2)]]
    payoffs = [{"Spot":sp,"P&L":round(premio if sp>=K_sel else premio-(K_sel-sp),4)} for sp in spots]
    st.line_chart(pd.DataFrame(payoffs).set_index("Spot"))

    if ticker_sel != "OUTRO":
        st.markdown("**📊 Gráfico do ativo · strike + níveis de ATR**")
        grafico_ativo(historico_diario_cache(ticker_sel, token),
                      strikes={f"Strike put R$ {K_sel:.2f}": K_sel},
                      niveis_atr=niveis_df, preco_ref=preco_ref)

    rule_box(["Usar apenas em ativos que você QUER ter","Strike = nível de suporte do ATR",
...
with tabs[3]:
    ...
    if ticker_sel != "OUTRO":
        st.markdown("**📊 Gráfico do ativo com os strikes do ciclo**")
        grafico_ativo(historico_diario_cache(ticker_sel, get_token()),
                      strikes={f"Venda PUT R$ {d_sel['K_put']:.2f}": d_sel["K_put"],
                               f"Venda CALL R$ {d_sel['K_call']:.2f}": d_sel["K_call"]})
...
with tabs[4]:
    ...
    for i, (col, d, label) in enumerate(zip(cols, dados, labels)):
        with col:
            detalhes = (f"Compra CALL K1 R$ {d['K1']:.2f} / Venda CALL K2 R$ {d['K2']:.2f}<br>"
    ...
    st.markdown(f"**Payoff: Compra CALL K1 R$ {t['K1']:.2f} → Venda CALL K2 R$ {t['K2']:.2f} · Débito R$ {t['debito']:.4f}**")
    ...
    if ticker_sel != "OUTRO":
        st.markdown("**📊 Gráfico do ativo com Compra/Venda CALL**")
        grafico_ativo(historico_diario_cache(ticker_sel, get_token()),
                      strikes={f"Compra CALL K1 R$ {t['K1']:.2f}": t["K1"],
                               f"Venda CALL K2 R$ {t['K2']:.2f}": t["K2"]})

    with st.expander("📋 Ver tabela completa"):
        df = pd.DataFrame([{
            "Perfil": l, "Vencimento": f"{d['dias']}d", "Largura": f"{d['largura_pct']}%",
            "Compra CALL (K1)": f"R$ {d['K1']:.2f}", "Venda CALL (K2)": f"R$ {d['K2']:.2f}",
...
with tabs[5]:
    ...
    for i, (col, d, label) in enumerate(zip(cols, dados, labels)):
        with col:
            detalhes = (f"Compra PUT K1 R$ {d['K1']:.2f} / Venda PUT K2 R$ {d['K2']:.2f}<br>"
                        f"Largura {d['largura_pct']}% · {d['dias']}d<br>"
                        f"R/R 1:{d['rr']}")
            clicado = card_perfil(label, f"R$ {d['lucro_max']:.2f}", detalhes,
    ...
    st.markdown(f"**Payoff: Compra PUT K1 R$ {tb['K1']:.2f} → Venda PUT K2 R$ {tb['K2']:.2f} · Débito R$ {tb['debito']:.4f}**")
    ...
    if ticker_sel != "OUTRO":
        st.markdown("**📊 Gráfico do ativo com Compra/Venda PUT**")
        grafico_ativo(historico_diario_cache(ticker_sel, get_token()),
                      strikes={f"Compra PUT K1 R$ {tb['K1']:.2f}": tb["K1"],
                               f"Venda PUT K2 R$ {tb['K2']:.2f}": tb["K2"]})

    with st.expander("📋 Ver tabela completa"):
        df = pd.DataFrame([{
            "Perfil": l, "Vencimento": f"{d['dias']}d", "Largura": f"{d['largura_pct']}%",
            "Compra PUT (K1)": f"R$ {d['K1']:.2f}", "Venda PUT (K2)": f"R$ {d['K2']:.2f}",
            "Débito": d["debito"], "Lucro máx": d["lucro_max"],
            "Break-even": f"R$ {d['breakeven']:.2f}", "R/R": f"1:{d['rr']}",
        } for d, l in zip(dados, labels)])
        st.dataframe(df, width="stretch", hide_index=True)
...
with tabs[6]:
    ...
    for i, (col, d, label) in enumerate(zip(cols, dados, labels)):
        with col:
            detalhes = (f"{d['nome']} · {d['dias']}d<br>"
                        f"Compra PUT R$ {d['K_put']:.2f} / Compra CALL R$ {d['K_call']:.2f}<br>"
    ...
    st.markdown(f"**Payoff: {ss['nome']} · Compra PUT R$ {ss['K_put']:.2f} + Compra CALL R$ {ss['K_call']:.2f} "
                f"· Custo total R$ {ss['custo_total']:.4f}**")
    ...
    if ticker_sel != "OUTRO":
        st.markdown("**📊 Gráfico do ativo com Compra PUT/Compra CALL**")
        grafico_ativo(historico_diario_cache(ticker_sel, get_token()),
                      strikes={f"Compra PUT R$ {ss['K_put']:.2f}": ss["K_put"],
                               f"Compra CALL R$ {ss['K_call']:.2f}": ss["K_call"]})

    with st.expander("📋 Ver tabela completa"):
        df = pd.DataFrame([{
            "Perfil": l, "Estrutura": d["nome"], "Vencimento": f"{d['dias']}d",
            "Compra PUT": f"R$ {d['K_put']:.2f}", "Compra CALL": f"R$ {d['K_call']:.2f}",
