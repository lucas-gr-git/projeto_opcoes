...
def grafico_ativo(df, strikes=None, niveis_atr=None, cor_atr="#EAB308", altura=380):
    """Candlestick do ativo, com linhas de strike da estratégia e, opcionalmente,
    níveis de ATR projetados (± N×ATR a partir do preço de referência)."""
    ...
    if strikes:
        ...
        if not df_k.empty:
            camadas.append(alt.Chart(df_k).mark_rule(strokeDash=[5, 3], strokeWidth=1.4, color="#4da6ff").encode(y="y:Q"))
            camadas.append(alt.Chart(df_k).mark_text(align="left", dx=4, dy=-6, color="#4da6ff", fontSize=10, fontWeight="bold")
                            .encode(y="y:Q", text="label:N", x=alt.value(3)))

    if niveis_atr is not None and not niveis_atr.empty:
        linhas = []
        for _, row in niveis_atr.iterrows():
            linhas.append({"y": row["preco_superior"], "label": f" +{row['rotulo']}"})
            linhas.append({"y": row["preco_inferior"], "label": f" −{row['rotulo']}"})
        df_atr = pd.DataFrame(linhas)
        camadas.append(alt.Chart(df_atr).mark_rule(strokeDash=[2, 2], strokeWidth=1, color=cor_atr).encode(y="y:Q"))
        camadas.append(alt.Chart(df_atr).mark_text(align="left", dx=4, dy=-5, color=cor_atr, fontSize=9)
                        .encode(y="y:Q", text="label:N", x=alt.value(3)))
...
with tabs[2]:
    ...
    if "atr_periodo" not in st.session_state:
        ...
        st.session_state.atr_niveis     = [0.5, 1.0, 2.0]
        st.session_state.atr_cor        = "#EAB308"
    ...
    with col_cfg.expander("⚙️ Configurar Níveis de ATR"):
        ...
        with st.form("form_atr_vp"):
            ...
            niveis_novo = st.multiselect("Níveis", NIVEIS_ATR_PADRAO,
                default=st.session_state.atr_niveis, format_func=lambda n: f"±{n:g}")
            cor_nova = st.color_picker("Cor", value=st.session_state.atr_cor)
        ...
        if salvar:
            ...
            st.session_state.atr_cor        = cor_nova
            st.session_state.atr_ativo      = True
            st.rerun()
        if remover:
    ...
    if ticker_sel != "OUTRO" and st.session_state.atr_ativo:
        ...
        if atr_valor:
            ...
            st.caption(f"📐 ATR({st.session_state.atr_periodo}) {st.session_state.atr_referencia} · "
                       f"valor: R$ {atr_valor:.2f} · referência: R$ {preco_ref:.2f}")
        else:
    ...
    if niveis_df is not None and not niveis_df.empty:
        ...
        rotulo_sel = st.radio("Nível ATR usado como strike", chaves, horizontal=True,
                              index=min(1, len(chaves) - 1), label_visibility="collapsed")
    ...
    if ticker_sel != "OUTRO":
        ...
        grafico_ativo(historico_diario_cache(ticker_sel, token),
                      ...
                      niveis_atr=niveis_df, cor_atr=st.session_state.atr_cor)
