...
def gerar_venda_coberta_variacoes(mkt: Mercado, n: int = 5) -> list:
    ...
    Ordem: Conservadora → Agressiva (strike mais longe do spot = menos prêmio,
    menor chance de exercício → strike mais perto do spot = mais prêmio, mais risco)."""
    mults = np_like_linspace(1.15, 1.03, n)
...
def gerar_wheel_variacoes(mkt: Mercado, n: int = 5) -> list:
    """Ordem: Conservadora → Agressiva (strikes mais afastados do spot dos dois
    lados = menos prêmio combinado → strikes mais perto do spot = mais prêmio)."""
    put_mults  = np_like_linspace(0.85, 0.97, n)
    call_mults = np_like_linspace(1.15, 1.03, n)
    dias_list  = _dias_variacoes(mkt.dias, n)
    resultados = []
...
def gerar_trava_alta_variacoes(mkt: Mercado, n: int = 5) -> list:
    """Bull Call Spread — Compra CALL (K1, ATM) + Venda CALL (K2, OTM).
...
def gerar_trava_baixa_variacoes(mkt: Mercado, n: int = 5) -> list:
    """Bear Put Spread — Compra PUT (K1, ATM) + Venda PUT (K2, OTM)."""
    larguras = np_like_linspace(0.03, 0.12, n)
    dias_list = _dias_variacoes(mkt.dias, n)
    resultados = []
    for largura, dias_v in zip(larguras, dias_list):
...
def gerar_straddle_strangle_variacoes(mkt: Mercado, n: int = 5) -> list:
    """Compra PUT + Compra CALL (straddle ATM até strangle largo).
    Ordem: Conservadora → Agressiva (strangle largo/barato, menor capital em risco
    → straddle ATM/caro, maior capital em risco — mesma convenção das outras abas)."""
    larguras = np_like_linspace(0.10, 0.0, n)
    dias_list = _dias_variacoes(mkt.dias, n)
    resultados = []
    for largura, dias_v in zip(larguras, dias_list):
        T_v = dias_v / 365
